...
public class DAPSol extends GameItem{
	
	
	public DAPSol(float posX,float posY,float width,float length,Context context){
		super(posX,posY);
		this.setPosX(posX);
		this.setPosY(posY);
		this.setWidth(width);
	...
	public void init() {
		...
		float[] textureCoords = {this.getLength(),this.getWidth(),
				0.0f,this.getWidth(),
				0.0f,0.0f,
				this.getLength(),0.0f,
				};
	...
	public void loadTexture() {
	    ...
	    bo.inScaled = false;
	    Bitmap tex = BitmapFactory.decodeResource(this.getContext().getResources(),R.drawable.sol, bo);
