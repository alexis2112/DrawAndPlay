...
public class GrosseGoutte extends GameItem{
		...
		public  GrosseGoutte(float posX,float posY,Context context){
			super(posX,posY);
			this.setPosX(posX);
			this.setPosY(posY);
			this.setWidth(0.7f);
		...
		public void init() {
			...
			float[] vertices = {this.getPosX(),this.getPosY(),0.0f,this.getPosX()+1.4f,this.getPosY(),0.0f,this.getPosX()+1.4f,this.getPosY()+0.7f,0.0f,this.getPosX(),this.getPosY()+0.7f,0.0f};
			...
			float[] textureCoords = {0.05f,1.0f,
					0.333f,1.0f,
					0.333f,0.25f,
					0.05f,0.25f,
		...
		public void loadTexture() {
		    ...
		    GLES20.glGenTextures(1, getTextures(), 0);

		    BitmapFactory.Options bo = new BitmapFactory.Options();
		    bo.inScaled = false;
		    Bitmap tex = BitmapFactory.decodeResource(this.getContext().getResources(),R.drawable.grossegouttesprite, bo);
		    GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, getTextures()[0]); 
		...
		public void fall(){
			if(!this.isDead()){
			this.checkEnvironmentDown();}else{blockedDown=false;}
			if(!blockedDown){
		...
		public void checkEnvironmentDown(){
			...
			length = item.getLength();
				...
				if(( x >xi-0.6f)&&( x <xi+length)&&(yi+width-0.07<=y)&&(y<=(yi+width+0.025f))){
					...
					this.maxX=item.getPosX()+item.getLength()-1.4f;}
		...
		public void checkEnvironmentRight(){
			...
			for(GameItem item : this.getRenderer().getDisplayList()){
				...
				//le blockedRight
				if(( x + 1.4f >xi)&&( x+1.4f <xi+0.1f)&&(yi<=y+0.7f)&&(y<=yi+width)){
				blockedRight=true;}
