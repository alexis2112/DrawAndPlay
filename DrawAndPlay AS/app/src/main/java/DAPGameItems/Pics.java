...
public class Pics extends GameItem{
	public Pics(float posX,float posY,float length,Context context){
		super(posX,posY);
		this.setPosX(posX);
		this.setPosY(posY);
		this.setWidth(0.4f);
	...
	public void init() {
		...
		float[] vertices = {this.getPosX(),this.getPosY(),0.0f,this.getPosX()+this.getLength(),this.getPosY(),0.0f,this.getPosX()+this.getLength(),this.getPosY()+0.4f,0.0f,this.getPosX(),this.getPosY()+0.4f,0.0f};
		...
		float[] textureCoords = {this.getLength()/1.5f,0.95f,
				0.0f,0.95f,
				0.0f,0.0f,
				this.getLength()/1.5f,0.0f,
				};
	...
	public void loadTexture() {
	    ...
	    bo.inScaled = false;
	    Bitmap tex = BitmapFactory.decodeResource(this.getContext().getResources(),R.drawable.pics, bo);
