...
public class PlateformeMobileVerticale extends GameItem{
	private float yUp;
	private float yDown;
	...
	public PlateformeMobileVerticale(float posX,float posY,float length,float yUp,float yDown,Context context){
		super(posX,posY);
		this.setPosX(posX);
		this.setPosY(posY);
		this.setWidth(0.3f);
	...
	public void updateCoordinates(){
		...
		if(this.goingUp){
		if(y<=yUp){this.getTranslation()[1]=this.getTranslation()[1]+0.01f;
		this.setPosY(this.getPosY()+0.01f);
		}else{this.goingUp=false;}
		}
		if(!this.goingUp){
		if(y>=yDown){this.getTranslation()[1]=this.getTranslation()[1]-0.01f;
		this.setPosY(this.getPosY()-0.01f);
		}else{this.goingUp=true;}
